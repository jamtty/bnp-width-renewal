import AnimatedSection from '../../components/AnimatedSection';
import { useState } from 'react';
import img_counseling from '../../assets/images/img_counseling_senior.png';
import img_counseling2 from '../../assets/images/img_counseling_senior2.png';
import img_counseling3 from '../../assets/images/img_counseling_senior3.png';
import ico_box_bg1 from '../../assets/images/ico_box_bg1.svg';
import ico_box_bg2 from '../../assets/images/ico_box_bg2.svg';
import ico_box_bg3 from '../../assets/images/ico_box_bg3.svg';
import ico_box_bg_time from '../../assets/images/ico_box_bg_time.svg';

const tabList = [
  '특장점',
  '시니어 진단 패키지',
  '상담 프로그램'
];

const CounselingChild = () => {
  const [activeTab, setActiveTab] = useState(0);

  return (
    <div className="page_cont page_sub child">
        <AnimatedSection className="page_tit" direction="up">
            <h2>시니어 상담</h2>
        </AnimatedSection>
        <section className="con_1">
            <div className="tab_wrap">
                <span className="tab_bar" style={{ left: `${(100 / tabList.length) * activeTab}%`, width: `${100 / tabList.length}%` }} />
                {tabList.map((tab, i) => (
                    <button
                        key={i}
                        type="button"
                        className={`tab_btn ${activeTab === i ? 'on' : ''}`}
                        onClick={() => setActiveTab(i)}
                    >{tab.split('\n').map((line, j) => (<>{j > 0 && <br />}{line}</>))}</button>
                ))}
            </div>
            <div className="tab_content">
                {activeTab === 0 && (
                    <>
                        <AnimatedSection className="tit_img" direction="up">
                            <img src={img_counseling} alt="성인 상담" />
                        </AnimatedSection>
                        <AnimatedSection className="tit" direction="up">
                            <h3>
                                은퇴 이후에도 삶은 계속됩니다<span className='pc_br'></span>
                                상실과 변화의 시간을 지나<span className='pc_br'></span>
                                다시 의미 있는 삶을 만들어 가는 상담입니다.
                            </h3>
                        </AnimatedSection>
                        <AnimatedSection className="box" direction="up">
                            <div className='s_tit'>
                                <h3>무엇이 다른가요?</h3>
                            </div>
                            <div className='s_info'>
                                <ul className='ul_list_1'>
                                    <li>사별·은퇴·건강 변화 이후의 심리 회복</li>
                                    <li>우울·외로움·무기력에 대한 전문 상담</li>
                                    <li>자녀·배우자와의 관계 갈등 상담</li>
                                    <li>노년기 삶의 의미와 역할 재설계</li>
                                    <li>가족과 함께하는 의사결정 및 돌봄 상담</li>
                                </ul>
                            </div>
                            <img src={ico_box_bg1} alt="" className="bg_img" />
                        </AnimatedSection>
                        <AnimatedSection className="box d-block" direction="up">
                            <div className='s_tit'>
                                <h3>상담은 어떻게 진행되나요?</h3>
                                <p>"나이가 들어도 삶은 계속됩니다."<br />정서와 관계를 회복하고 다시 의미 있는 삶을 만들어 가는 상담입니다.</p>
                            </div>
                            <div className='s_info'>
                                <ul className='step_list_child'>
                                    <li>
                                        <div className='ico'>
                                            <p>초기 면담</p>
                                        </div>
                                        <p className='txt'>현재 갈등과 관계 흐름 이해</p>
                                    </li>
                                    <li>
                                        <div className='ico'>
                                            <p>시니어 진단</p>
                                        </div>
                                        <p className='txt'>정서·인지·관계 상태 확인</p>
                                    </li>
                                    <li>
                                        <div className='ico'>
                                            <p>해석 상담</p>
                                        </div>
                                        <p className='txt'>현재 상태와 원인 설명</p>
                                    </li>
                                    <li>
                                        <div className='ico'>
                                            <p>상담 방향 설정</p>
                                        </div>
                                        <p className='txt'>회복 및 변화 방향 설정</p>
                                    </li>
                                    <li>
                                        <div className='ico'>
                                            <p>상담 진행</p>
                                        </div>
                                        <p className='txt'>정서 회복 + 관계 조율<br /> + 삶의 통합</p>
                                    </li>
                                </ul>
                            </div>
                        </AnimatedSection>
                        <AnimatedSection className="box" direction="up">
                            <div className='s_tit'>
                                <h3>상담 시간 안내</h3>
                            </div>
                            <div className='s_info'>
                                <ul className='ul_list_3'>
                                    <li>초기 면담 : 50분</li>
                                    <li>시니어 상담: 50분</li>
                                    <li>심리검사: 60~120분</li>
                                    <li>해석 상담: 50분</li>
                                    <li>상황에 따라 유연하게 조정됩니다.</li>
                                </ul>
                            </div>
                            <img src={ico_box_bg_time} alt="" className="bg_img" />
                        </AnimatedSection>
                    </>
                )}
                {activeTab === 1 && (
                    <>
                        <AnimatedSection className="tit_img" direction="up">
                            <img src={img_counseling2} alt="시니어 진단 패키지" />
                        </AnimatedSection>
                        <AnimatedSection className="box d-block-new" direction="up">
                            <div className='s_tit'>
                                <h3>시니어 종합 마음건강 진단</h3>
                            </div>
                            <div className='s_info'>
                                <ul className='ul_list_Package_1'>
                                    <li>우울·무기력 상태 확인</li>
                                    <li>기억력 및 인지 기능 점검</li>
                                    <li>삶의 만족도와 의미 수준 평가</li>
                                    <li>자녀·배우자와의 관계 상태 점검</li>
                                    <li>전문 해석 상담 포함</li>
                                </ul>
                                <p className='Package-txt-blue'>정서·인지·관계·삶의 만족도를 종합적으로 확인하여<span className='pc_br'></span>현재의 상태와 회복 방향을 찾습니다.</p>
                            </div>
                            <img src={ico_box_bg1} alt="" className="bg_img" />
                        </AnimatedSection>
                        <AnimatedSection className="box d-block-new" direction="up">
                            <div className='s_tit'>
                                <h3>기억·인지 기능 정밀 진단</h3>
                            </div>
                            <div className='s_info'>
                                <ul className='ul_list_Package_2'>
                                    <li>기억력 및 주의력 평가</li>
                                    <li>인지 기능 전반 점검</li>
                                    <li>일상생활 수행 능력 확인</li>
                                    <li>치매 위험도 및 초기 변화 확인</li>
                                    <li>전문 해석 상담 포함</li>
                                </ul>
                                <p className='Package-txt-blue'>기억력 저하의 원인을 확인하고 필요한 대응 방향을 안내합니다.</p>
                            </div>
                            <img src={ico_box_bg2} alt="" className="bg_img" />
                        </AnimatedSection>
                        <AnimatedSection className="box d-block-new" direction="up">
                            <div className='s_tit'>
                                <h3>정서·상실 반응 진단</h3>
                            </div>
                            <div className='s_info'>
                                <ul className='ul_list_Package_3'>
                                    <li>우울·불안 수준 평가</li>
                                    <li>상실과 애도 반응 점검</li>
                                    <li>스트레스 및 정서 상태 확인</li>
                                    <li>삶의 의미와 만족도 평가</li>
                                    <li>전문 해석 상담 포함</li>
                                </ul>
                                <p className='Package-txt-blue'>사별, 외로움, 우울감, 무기력 등 노년기 정서 상태를 확인합니다.</p>
                            </div>
                            <img src={ico_box_bg3} alt="" className="bg_img" />
                        </AnimatedSection>
                    </>
                )}
                {activeTab === 2 && (
                    <>
                        <AnimatedSection className="tit_img" direction="up">
                            <img src={img_counseling3} alt="상담 프로그램" />
                        </AnimatedSection>
                        <AnimatedSection className="box d-block-new" direction="up">
                            <div className='s_tit'>
                                <h3>상실 회복 프로그램</h3>
                            </div>
                            <div className='s_info'>
                                <ul className='ul_list_Package_1'>
                                    <li>감정 정리</li>
                                    <li>회상 치료</li>
                                    <li>삶의 의미 재구성</li>
                                </ul>
                                <p className='Package-txt-blue'>사별 · 외로움 · 무의미감 상실의 감정을 이해하고 애도하며 삶의 의미를 다시 연결합니다.</p>
                            </div>
                            <img src={ico_box_bg1} alt="" className="bg_img" />
                        </AnimatedSection>
                        <AnimatedSection className="box d-block-new" direction="up">
                            <div className='s_tit'>
                                <h3>가족 관계 회복 프로그램</h3>
                            </div>
                            <div className='s_info'>
                                <ul className='ul_list_Package_2'>
                                    <li>자녀 관계 회복</li>
                                    <li>가족 간 의사소통</li>
                                    <li>돌봄 부담 조율</li>
                                    <li>가족 역할 재정립</li>
                                </ul>
                                <p className='Package-txt-blue'>자녀 갈등 · 부부 갈등 · 돌봄 스트레스 세대 간의 차이와 쌓인 감정을 이해하고,<span className='pc_br'></span>가족 안에서 건강한 관계와 역할을 다시 만들어갑니다.</p>
                            </div>
                            <img src={ico_box_bg2} alt="" className="bg_img" />
                        </AnimatedSection>
                        <AnimatedSection className="box d-block-new" direction="up">
                            <div className='s_tit'>
                                <h3>인지·기억 유지 프로그램</h3>
                            </div>
                            <div className='s_info'>
                                <ul className='ul_list_Package_3'>
                                    <li>기억력 자극</li>
                                    <li>주의집중력 강화</li>
                                    <li>인지 활동</li>
                                    <li>일상 기능 유지</li>
                                </ul>
                                <p className='Package-txt-blue'>기억력 저하 · 인지 변화에 대한 걱정 현재의 인지 기능을 바탕으로 기억과 주의력을 자극하고,<span className='pc_br'></span>안정적인 일상생활을 유지하도록 돕습니다.</p>
                            </div>
                            <img src={ico_box_bg3} alt="" className="bg_img" />
                        </AnimatedSection>
                    </>
                )}
            </div>
            <AnimatedSection className="btn_wrap" direction="up">
                <button type="button" className='btn_primary'>시니어 상담 신청하기</button>
                <button type="button" className='btn_secondary'>검사 &amp; 해석 상담 신청하기</button>
            </AnimatedSection>
        </section>
    </div>
  );
};
export default CounselingChild;
